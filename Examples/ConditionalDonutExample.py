...
parser.add_argument("--grad_steps", type=int, default=1000)
...
if load_path is None:
    ...
    # train the model
    marginal_distribution_model.train_model(dataloader, grad_steps=grad_steps, callback=callback)
    ...
    # training loop
    for epoch in trange(grad_steps):
        with torch.no_grad():
...
with torch.no_grad():
    ...
    _, _, _, _, info = next(iter(dataloader))
    ...
    for i in range(9):
        ...
        circle = plt.Circle((0, 0), radii[i].cpu().detach(), color='b', fill=False)
