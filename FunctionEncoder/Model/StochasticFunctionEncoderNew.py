...
class StochasticFunctionEncoderNew(torch.nn.Module):
    ...
    def __init__(self,
        ...
        super(StochasticFunctionEncoderNew, self).__init__()
    ...
    def compute_representation(self, example_xs, example_ys, method="inner_product", **kwargs):
        ...
        if method == "inner_product":
            ...
            outs = self._compute_grad_mle(example_xs, example_ys, **kwargs)
            GTG = None
        elif method == "mle":
            outs = self._compute_mle(example_xs, example_ys, **kwargs)
            GTG = None
        elif method == "osip":
            outs, GTG = self._compute_osip(example_xs, example_ys, **kwargs)
        else:
    ...
    def _compute_grad_mle(self, example_xs, example_ys, grad_steps=100_000):
    ...
    def _compute_mle(self, example_xs, example_ys):
        G_xs = self.forward(None, example_ys)
        G_mean = torch.mean(G_xs, dim=1) # take mean over data points
        representation = G_mean / torch.clamp(torch.norm(G_mean, dim=-1, keepdim=True), min=1e-7)
        return representation

    def _compute_osip(self, example_xs, example_ys):
        # first sample a bunch of random points to compute means for Gs
        random_points = self.sample(1, example_ys.shape[1] * example_ys.shape[0], example_ys.device)
        G_random = self.forward(None, random_points)
        means = torch.mean(G_random, dim=1)
        assert means.shape == (1, self.n_basis)

        # next compute logits for all example data
        # and subtract means to remove scaling
        example_logits = self.forward(None, example_ys)
        recented_example_logits = example_logits - means

        # Now, use to compute representation
        representation = torch.mean(recented_example_logits, dim=1)
        return representation, means
    ...
    def train_model(self,
        ...
        method = self.method
        assert method in ["inner_product", "least_squares", "mle", "osip"], f"Unknown method: {method}"
        ...
        for epoch in bar:
            ...
            if method == "mle":
                # compute mle representation, constrained to unit ball, and compute log probs
                representation, _ = self.compute_representation(example_xs, example_ys, method="mle")
                log_probs = self.predict(xs, ys, representation)
                logit_loss = -torch.mean(log_probs)

                # compute magnitude of every representation, and force average value to 0
                # diverse_xs = torch.rand(10_000, ys.shape[2]) * (dataset.highs - dataset.lows) + dataset.lows
                # diverse_logits = self.forward(None, diverse_xs.to(device))
                # integrals = diverse_logits.mean(dim=0) * self.volume
                # mean_value_loss = torch.mean(integrals ** 2)

                # try forcing int e^cg(x) = 1
                diverse_xs = torch.rand(10_000, ys.shape[2]) * (dataset.highs - dataset.lows) + dataset.lows
                diverse_logits = self.forward(None, diverse_xs.to(device))
                function_logits = torch.einsum("dk,fk->fd", diverse_logits, representation)
                e_logits = torch.exp(function_logits)
                integral = torch.mean(e_logits, dim=1) * self.volume
                mean_value_loss = torch.mean((integral - 1) ** 2)
                loss = logit_loss + 100 * mean_value_loss
            if method == "osip":

                # nll loss
                rep, means = self.compute_representation(example_xs, example_ys, method="osip")
                log_probs = self.predict(xs, ys, rep)
                logit_loss = -torch.mean(log_probs)

                # mean loss
                mean_loss = (means**2).mean()

                writer.add_scalar("train/means", mean_loss.item(), epoch)

                loss = logit_loss + 1e3 * mean_loss
            ...
            if logdir is not None:
                ...
                if method == "mle":
                    writer.add_scalar("train/mean_value_loss", mean_value_loss.item(), epoch)